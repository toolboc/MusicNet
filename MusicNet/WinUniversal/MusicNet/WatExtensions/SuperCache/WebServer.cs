namespace WatExtensions.SuperCache
{
    ...
    internal sealed class WebServer : IDisposable
    {
        ...
        public static string BuildProxyUri(Uri proxyUri, Uri baseUri, string requestUri, IEnumerable<string> bypassUrlPatterns)
        {
            ...
            {
                var targetHost = new Uri(new Uri(baseUri, requestUri).GetComponents(UriComponents.SchemeAndServer, UriFormat.UriEscaped));

                if (!(string.Equals(targetHost.Scheme, "http", StringComparison.OrdinalIgnoreCase) || string.Equals(targetHost.Scheme, "https", StringComparison.OrdinalIgnoreCase)) || BypassUrl(requestUri, bypassUrlPatterns))
                {
                    return requestUri;
                }
                                ...
                                if (handler != null)
                                {
                                    if (this.context != null)
                                    {
                                        this.context.Post(
                                            (state) =>
                                            {
                                                handler(this, new OfflinePageUnavailableEventArgs(request.RequestUri));
                                            }, 
                                            null);
                                    }
                                }
        ...
        private async Task OnSendingRequestAsync(HttpRequestMessage request, Guid requestId)
        {
            ...
            {
                var eventArgs = new SendingRequestEventArgs(request);
            ...
            if (content != null && content.Headers.ContentType != null)
            {
                ...
                if (mediaType == "text" || (mediaType == "application" && (mediaSubType == "json" || mediaSubType == "xhtml+xml")))
                {
                    ...
                    if (mediaType == "text" || mediaType == "application")
                    {
                        ...
                        {
                            var processor = new HtmlProcessor(htmlContent, this, this.baseUri, this.configuration.BypassUrlPatterns);
